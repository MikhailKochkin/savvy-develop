...
class App extends React.Component {
  ...
  hasBlock = type => {
    const { value } = this.state;
    // console.log(value.blocks.map(node => console.log(node.type)));
  ...
  hasComments = () => {
    const { value } = this.state;
    // console.log(value.inlines.some(inline => inline.type === "comment"));
    ...
    return (
      <>
        <FormatToolBar>
          ...
          <ButtonStyle onMouseDown={event => this.onClickCode(event, "code")}>
            <Icon icon={code} />
          </ButtonStyle>
  ...
  onClickCode = (event, type) => {
    event.preventDefault();
    const isCode = this.hasCodeBlock(type);
    if (isCode) {
      this.editor.unwrapBlock("code").wrapBlock("paragraph");
    } else {
      this.editor
