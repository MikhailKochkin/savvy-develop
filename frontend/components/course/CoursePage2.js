...
const CoursePage = (props) => {
  ...
  let weeks;
  coursePage.weeks ? (weeks = coursePage.week) : (weeks = 3);

  const new_subjectArray = [];
  me &&
    me.new_subjects.map((new_subject) => new_subjectArray.push(new_subject.id));
          ...
          <LessonStyles>
                ...
                {coursePage.authors.length > 0 ? (
                  coursePage.authors.map((a) => (
                    <div className="name">
                      <img src={a.image} />
                      <p>
                        {a.name} {a.surname} {props.t("from")} {a.company.name}
                      </p>
                    </div>
                  ))
                ) : (
              ...
              <PayBox>
                ...
                {me &&
                  ...
                  !me.permissions.includes("ADMIN") && (
                    <FirstLesson
                      lesson={
                        coursePage.lessons.filter((l) => l.open === true)[0]
                      }
                    />
                  )}{" "}
                {/* Карточка первого урока */}
            ...
            <LessonsInfo>
              <Buttons>
                ...
                >
                  {props.t("feedbackPage")}
                </Button>
              </Buttons>
              {page === "lessons" && (
                <>
                  <Total>
                    {props.t("total")} {coursePage.lessons.length}
                  </Total>
                  {coursePage.lessons
                ...
                (me.id === coursePage.user.id ||
                  ...
                  <>
                    {me.studentFeedback.filter((feed) =>
                      ...
                      <p>{props.t("no_feedback")}</p>
                    ) : null}
                    {me.studentFeedback
                      .filter((feed) => lessonsList.includes(feed.lesson.id))
                      .map((feedback) => (
                        <Feedback feedback={feedback} />
                      ))}
                  </>
                ) : (
                  <Comment>{props.t("signup")}</Comment>
                ))}
            </LessonsInfo>
            <Details>
